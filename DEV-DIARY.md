...
## 2026-07-11 技能和工具页按当前入口智能体收敛显示

### 问题
“技能库”和“插件”页之前都是全量展示。

这会导致：
- 当前入口智能体不是 Codex 时，插件页仍然可能出现 `codex` 来源的 MCP 插件
- 卡片上写着“已启用”，但用户无法判断到底是“对谁启用”
- 技能库里也会混入当前入口智能体暂时无关的技能

### 修改文件

| 文件 | 改动 |
|------|------|
| `src/components/skills-and-tools/skills-and-tools-page.tsx` | 让“技能库”只显示当前入口智能体已启用的技能；让“插件”只显示 `apps` 包含当前入口智能体的 MCP 插件；插件卡片文案改为“已对当前智能体启用”语义 |

### 调整结果
- **技能库**：只展示当前入口智能体已经启用 / 已关联的技能卡片
- **插件**：只展示与当前入口智能体相关的 MCP 插件
- **插件状态文案**：从模糊的“已启用”改成面向当前入口智能体的表达，避免用户误以为是启用给别的智能体
