 ...
 * is injected only into the spawned pi child, never into veryagent's own process.
 */
 ...
 *
 * veryagent's Settings skill store (Skills tab, Experts, Office tools) manages each
 ...
 * but not veryagent's process), so the skill store can't target it. Surfacing such
