...
//!    the default level from the `logging.level` KV value (unless `VERYAGENT_LOG` /
...
/// overridable via `VERYAGENT_LOG_MAX_FILES`.
fn file_retention() -> usize {
    std::env::var("VERYAGENT_LOG_MAX_FILES")
        .ok()
...
/// `VERYAGENT_LOG` then `RUST_LOG`. Single source of truth for env-level precedence,
/// so the startup filter and the env-lock checks can't diverge — e.g. an empty
/// `VERYAGENT_LOG` must not mask a valid `RUST_LOG` (a `var().or_else(var)` would
/// treat the empty `Ok("")` as present and skip `RUST_LOG`).
fn env_level_override() -> Option<String> {
    ["VERYAGENT_LOG", "RUST_LOG"].iter().find_map(|key| {
        std::env::var(key)
            .ok()
...
/// Whether an explicit `VERYAGENT_LOG` / `RUST_LOG` is set (and non-empty). When
...
/// value, unless an explicit `VERYAGENT_LOG` / `RUST_LOG` is set (env wins). No-op
...
mod tests {
    ...
    fn env_level_override_precedence() {
        ...
        temp_env::with_vars(
            [("VERYAGENT_LOG", None::<&str>), ("RUST_LOG", None::<&str>)],
            || {
                assert_eq!(env_level_override(), None);
                assert!(!env_level_is_set());
            },
        );
        // Empty VERYAGENT_LOG must NOT mask a valid RUST_LOG (the bug Codex caught).
        temp_env::with_vars(
            [("VERYAGENT_LOG", Some("")), ("RUST_LOG", Some("debug"))],
            || {
        ...
        temp_env::with_vars(
            [("VERYAGENT_LOG", Some("  ")), ("RUST_LOG", Some(""))],
            || {
                assert_eq!(env_level_override(), None);
                assert!(!env_level_is_set());
            },
        );
        // VERYAGENT_LOG wins when both are non-empty.
        temp_env::with_vars(
            [("VERYAGENT_LOG", Some("trace")), ("RUST_LOG", Some("debug"))],
            || {
