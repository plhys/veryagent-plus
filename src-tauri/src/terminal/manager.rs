...
/// flags and the `eval "$VERYAGENT_CMD"` wrapping for shells we know speak that
        ...
        match detect_windows_shell_flavor(shell) {
            WindowsShellFlavor::Cmd => {
                if let Some(command) = initial_command {
                    cmd.env("VERYAGENT_CMD", command);
                    // Set UTF-8 code page before running the actual command
                    cmd.args(["/D", "/S", "/C", "chcp 65001 >nul & %VERYAGENT_CMD%"]);
                } else {
            ...
            WindowsShellFlavor::PowerShell => {
                if let Some(command) = initial_command {
                    ...
                    cmd.args([
                        ...
                        "-NoProfile",
                        "-Command",
                        "[Console]::OutputEncoding = [System.Text.Encoding]::UTF8; $ErrorActionPreference = 'Stop'; Invoke-Expression $env:VERYAGENT_CMD",
                    ]);
                } else {
            ...
            WindowsShellFlavor::Posix => {
                ...
                if let Some(command) = initial_command {
                    cmd.env("VERYAGENT_CMD", command);
                    cmd.args(["-l", "-i", "-c", "eval \"$VERYAGENT_CMD\""]);
                } else {
                    cmd.args(["-l", "-i"]);
                }
            }
