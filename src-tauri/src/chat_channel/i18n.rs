...
pub fn permission_request_body(lang: Lang) -> &'static str {
    match lang {
        Lang::ZhCn => "智能体正在请求权限，请在 VeryAgent 中查看并批准。",
        Lang::ZhTw => "智慧代理正在請求權限，請在 VeryAgent 中查看並批准。",
        Lang::Ja => "エージェントが権限を要求しています。VeryAgent で確認して承認してください。",
        Lang::Ko => "에이전트가 권한을 요청하고 있습니다. VeryAgent에서 확인하고 승인하세요.",
        Lang::Es => "Un agente solicita permiso. Revísalo y apruébalo en VeryAgent.",
        Lang::De => "Ein Agent fordert eine Berechtigung an. Bitte in VeryAgent prüfen und genehmigen.",
        Lang::Fr => "Un agent demande une autorisation. Vérifiez-la et approuvez-la dans VeryAgent.",
        Lang::Pt => "Um agente está solicitando permissão. Revise e aprove no VeryAgent.",
        Lang::Ar => "يطلب أحد الوكلاء إذنًا. يرجى مراجعته والموافقة عليه في VeryAgent.",
        Lang::En => "An agent is requesting permission. Review and approve it in VeryAgent.",
    }
}
...
pub fn question_request_body(lang: Lang) -> &'static str {
    match lang {
        Lang::ZhCn => "智能体正在向你提问，请在 VeryAgent 中回答。",
        Lang::ZhTw => "智慧代理正在向你提問，請在 VeryAgent 中回答。",
        Lang::Ja => "エージェントが質問しています。VeryAgent で回答してください。",
        Lang::Ko => "에이전트가 질문하고 있습니다. VeryAgent에서 답변하세요.",
        Lang::Es => "Un agente te hace una pregunta. Respóndela en VeryAgent.",
        Lang::De => "Ein Agent stellt eine Frage. Bitte in VeryAgent beantworten.",
        Lang::Fr => "Un agent vous pose une question. Répondez-y dans VeryAgent.",
        Lang::Pt => "Um agente está fazendo uma pergunta. Responda no VeryAgent.",
        Lang::Ar => "يطرح أحد الوكلاء سؤالاً. يرجى الإجابة عليه في VeryAgent.",
        Lang::En => "An agent is asking a question. Answer it in VeryAgent.",
    }
}
...
pub fn help_title(lang: Lang) -> &'static str {
    match lang {
        Lang::ZhCn => "VeryAgent Bot 帮助",
        Lang::ZhTw => "VeryAgent Bot 幫助",
        Lang::Ja => "VeryAgent Bot ヘルプ",
        Lang::Ko => "VeryAgent Bot 도움말",
        Lang::Es => "Ayuda de VeryAgent Bot",
        Lang::De => "VeryAgent Bot Hilfe",
        Lang::Fr => "Aide VeryAgent Bot",
        Lang::Pt => "Ajuda do VeryAgent Bot",
        Lang::Ar => "مساعدة VeryAgent Bot",
        Lang::En => "VeryAgent Bot Help",
    }
}
