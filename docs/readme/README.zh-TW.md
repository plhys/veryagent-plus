...
VeryAgent（Code Generation）是一個多智慧體編碼工作台，它將多個智慧體（Claude Code、Codex CLI、OpenCode、Gemini CLI、OpenClaw、Cline、Hermes Agent、CodeBuddy、Kimi Code、Pi 等）統一到一個工作區中，支援會話彙整和多智慧體協作，支援桌面安裝、伺服器/Docker 部署。
...
<table>
  <tr>
    <td colspan="2" align="center">
      ...
      <strong><a href="https://myclaw.ai/?utm_source=github&utm_campaign=veryagent">MyClaw.ai</a></strong> — 全託管的 OpenClaw 雲端執行個體服務，一鍵部署、7×24 全天候在線、資料完全由使用者掌控，無需自行管理伺服器。
    </td>
  </tr>
  <tr>
    <td align="center" width="220">
      <a href="https://www.compshare.cn/?ytag=GPU_YY_git_veryagent" target="_blank"><img src="../images/compshare.png" alt="優雲智算" width="160" /></a><br/>
      <strong><a href="https://www.compshare.cn/?ytag=GPU_YY_git_veryagent">優雲智算</a></strong>
    </td>
    <td>感謝優雲智算贊助了本專案！優雲智算是 UCloud 旗下 AI 雲平台，主打包月、按次的高性價比國模 agent Plan 套餐，低至 49 元/月起。同時提供官轉穩定海外模型。支援接入 Claude Code、Codex 及 API 呼叫。支援企業高併發、7*24 技術支援、自助開票。透過<a href="https://www.compshare.cn/?ytag=GPU_YY_git_veryagent">此連結</a>註冊的使用者，可得免費 5 元平台體驗金！</td>
  </tr>
  <tr>
    <td align="center" width="220">
...
> 想成為 VeryAgent 贊助商？[歡迎透過郵件與我們聯絡。](mailto:itpkcn@gmail.com)
...
- Web 服務模式 — 開啟後可在瀏覽器中存取 VeryAgent，支援遠端工作
- **獨立伺服器部署** — 在任意 Linux/macOS 伺服器上執行 `veryagent-server`，透過瀏覽器存取
...
- **無縫整合** — 新建立的專案會立即在 VeryAgent 工作台中開啟
...
將 Word、Excel 和 PowerPoint 文件納入一等工作流程。內建的 **officecli** 工具集讓你的智慧體能夠建立、分析、校對和編輯 .docx、.xlsx、.pptx 文件——並可直接在 VeryAgent 內預覽結果。
...
VeryAgent 在單一 workspace 中提供三個 Rust 二進位檔：

| 二進位         | 角色                                                                                          | 建置方式                                                                    |
| -------------- | --------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| `veryagent`        | Tauri 桌面應用程式（視窗、系統匣、自動更新）                                                  | `pnpm tauri build`（發行）/ `pnpm tauri dev`（開發）                        |
| `veryagent-server` | 用於瀏覽器/無頭部署的獨立 HTTP + WebSocket 伺服器                                             | `pnpm server:build` / `pnpm server:dev`                                     |
| `veryagent-mcp`    | 單次啟動的 stdio MCP 協作行程，向 agent CLI 公開 `delegate_to_agent` 工具（多智慧體協作）     | `pnpm tauri:prepare-sidecars`（由 `tauri dev` / `tauri build` 自動呼叫）    |

`veryagent-mcp` 在執行階段必須與其父二進位位於同一目錄——安裝程式、Docker 映像和 Tauri sidecar 打包工具都會將它放在 `veryagent` / `veryagent-server` 旁邊。原始碼建置和自訂部署可以透過 `VERYAGENT_MCP_BIN=/abs/path/veryagent-mcp` 環境變數覆寫查詢路徑。若協作行程缺失，委派功能會被略過（僅記錄一則警告日誌），其餘 agent 會話仍可正常運作。
...
# 完整桌面應用（Tauri + Next.js，自動建置 veryagent-mcp sidecar）
pnpm tauri dev

# 桌面發行建置（將 veryagent-mcp 作為 externalBin 打包）
pnpm tauri build

# 獨立伺服器（無需 Tauri/GUI）
pnpm server:dev
pnpm server:build                  # 發行二進位位於 src-tauri/target/release/veryagent-server

# 顯式建置 veryagent-mcp 協作行程（針對當前主機 triple）
pnpm tauri:prepare-sidecars        # 輸出：src-tauri/binaries/veryagent-mcp-<triple>
...
cargo check --no-default-features --bin veryagent-server            # 伺服器模式
cargo check --no-default-features --bin veryagent-mcp               # MCP 協作行程
cargo clippy --all-targets --features test-utils -- -D warnings

# Rust 測試
cargo test --features test-utils                                # 桌面（含整合）
cargo test --no-default-features --bin veryagent-server --lib       # 伺服器模式
cargo insta review                                              # 接受解析器快照變更
```

> 提示：當你在 `src-tauri/target/release/` 下有新建置的 `veryagent-mcp` 並想讓手動啟動的 `veryagent-server` 在不重新安裝的情況下指向它時，可以匯出 `VERYAGENT_MCP_BIN=$(pwd)/src-tauri/target/release/veryagent-mcp`。

### 伺服器部署

VeryAgent 可以作為獨立 Web 伺服器執行，無需桌面環境。
...
預建置二進位檔（已打包 Web 前端資源）可在 [Releases](https://github.com/plhys/veryagent-plus/releases) 頁面下載：
...
cargo build --release --bin veryagent-mcp --no-default-features    # 委派協作行程
VERYAGENT_STATIC_DIR=../out ./target/release/veryagent-server          # veryagent-mcp 會作為同級二進位被自動探測
```

> 若兩個二進位分別存放在不同目錄，請設定 `VERYAGENT_MCP_BIN=/abs/path/to/veryagent-mcp`，執行階段才能找到協作行程；否則多智慧體委派會被靜默停用。
...
| 變數                           | 預設值                 | 說明                                                                                                                                                                                                                                                                                                              |
| ------------------------------ | ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `VERYAGENT_PORT`                   | `3080`                 | HTTP 連接埠                                                                                                                                                                                                                                                                                                       |
| `VERYAGENT_HOST`                   | `0.0.0.0`              | 綁定位址                                                                                                                                                                                                                                                                                                          |
| `VERYAGENT_TOKEN`                  | _（隨機）_             | 認證令牌（啟動時輸出到 stderr）                                                                                                                                                                                                                                                                                   |
| `VERYAGENT_DATA_DIR`               | `~/.local/share/veryagent` | SQLite 資料庫目錄（同時也是 `uploads/`、`pets/` 的根目錄）                                                                                                                                                                                                                                                        |
| `VERYAGENT_STATIC_DIR`             | `./web` 或 `./out`     | Next.js 靜態匯出目錄                                                                                                                                                                                                                                                                                              |
| `VERYAGENT_MCP_BIN`                | _（未設定）_           | `veryagent-mcp` 協作行程的絕對路徑。會覆寫預設的「可執行檔同級目錄 + `PATH`」查詢邏輯。用於原始碼建置或協作行程不在伺服器安裝目錄內的自訂部署。                                                                                                                                                                       |
| `VERYAGENT_SKIP_SIDECAR`           | _（未設定）_           | 僅供 `pnpm tauri dev` / `pnpm tauri build` 調試前端時使用——當值為 `1` 時，略過 `veryagent-mcp` sidecar 的建置。此類建置不支援委派功能；發行品質的產出物必須保持此變數未設定。                                                                                                                                          |
| `VERYAGENT_UPLOAD_MAX_TOTAL_BYTES` | _（未設定）_           | `<data dir>/uploads/` 下所有檔案總位元組數的硬上限。十進位位元組數（例如 `10737418240` 表示 10 GiB）。未設定、`0` 或無法解析的值會停用上限，並在啟動時印出一行日誌以便觀察當前狀態。該上限僅在單一 `veryagent-server` 行程內生效——共用同一個 `uploads/` 卷的橫向擴展部署需要外部協調（檔案鎖、Redis、反向代理配額）。 |
| `VERYAGENT_UPLOAD_QUOTA_STRICT`    | _（未設定）_           | 當值為真（`1` / `true` / `yes` / `on`）時，若 `VERYAGENT_UPLOAD_MAX_TOTAL_BYTES` 設定為無法解析的值，則以結束代碼 2 中止啟動，而不是發出 WARN 後繼續執行。當安全政策要求「設定的配額必須生效」時使用此選項。                                                                                                          |
...
- 如果 VeryAgent 對你有幫助，不如請我喝杯咖啡吧

<img src="../images/weixin-sponsor-light.jpg#gh-light-mode-only" alt="贊助 VeryAgent" width="240" />
<img src="../images/weixin-sponsor-dark.jpg#gh-dark-mode-only" alt="贊助 VeryAgent" width="240" />

## 致謝

- [ACP](https://agentclientprotocol.com)：智能體客戶端協定 (ACP) 是 veryagent 實現多智能體連接的基礎
- [Superpowers](https://github.com/obra/superpowers)：為 VeryAgent 的專家技能模組提供支援
- [OfficeCLI](https://github.com/iOfficeAI/OfficeCLI)：為 VeryAgent 的 Office 文件工作流程提供支援
