...
VeryAgent (Code Generation) هو مساحة عمل للبرمجة متعددة الوكلاء. يجمع عدة وكلاء (Claude Code، Codex CLI، OpenCode، Gemini CLI، OpenClaw، Cline، Hermes Agent، CodeBuddy، Kimi Code، Pi، وغيرها) في مساحة عمل واحدة، ويدعم تجميع المحادثات والتعاون بين عدة وكلاء، مع دعم التثبيت على سطح المكتب والنشر على الخادم/Docker.
...
<table>
  <tr>
    <td colspan="2" align="center">
      ...
      <strong><a href="https://myclaw.ai/?utm_source=github&utm_campaign=veryagent">MyClaw.ai</a></strong> — منصة OpenClaw سحابية مُدارة بالكامل: نشر بنقرة واحدة، وتشغيل على مدار الساعة طوال أيام الأسبوع، وملكية كاملة للبيانات — دون الحاجة إلى إدارة أي خادم.
    </td>
  </tr>
  <tr>
...
> هل ترغب في أن تصبح راعياً لـ VeryAgent؟ [راسلنا عبر البريد الإلكتروني.](mailto:itpkcn@gmail.com)
...
- وضع خدمة الويب — الوصول إلى VeryAgent من أي متصفح للعمل عن بُعد
- **نشر خادم مستقل** — شغّل `veryagent-server` على أي خادم Linux/macOS، والوصول عبر المتصفح
...
- **تكامل سلس** — يُفتح المشروع المُنشأ حديثًا مباشرة في مساحة عمل VeryAgent
...
تعامَل مع ملفات Word وExcel وPowerPoint كجزء أصيل من سير العمل. تتيح مجموعة أدوات **officecli** المدمجة لوكلائك إنشاء وتحليل ومراجعة وتحرير مستندات .docx و.xlsx و.pptx — مع إمكانية معاينة النتائج مباشرةً داخل VeryAgent.
...
يوفّر VeryAgent ثلاثة ملفات تنفيذية بلغة Rust من workspace واحد:

| الملف التنفيذي | الدور                                                                                                                  | البناء                                                                       |
| -------------- | ---------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| `veryagent`        | تطبيق سطح المكتب Tauri (نافذة، شريط النظام، المُحدِّث)                                                                 | `pnpm tauri build` (إصدار) / `pnpm tauri dev` (تطوير)                        |
| `veryagent-server` | خادم HTTP + WebSocket مستقل لعمليات النشر عبر المتصفح/بدون واجهة                                                       | `pnpm server:build` / `pnpm server:dev`                                      |
| `veryagent-mcp`    | رفيق MCP عبر stdio يُشغَّل لكل جلسة، ويُتيح أداة `delegate_to_agent` لواجهات CLI للوكلاء (التعاون متعدد الوكلاء)        | `pnpm tauri:prepare-sidecars` (يُستدعى تلقائيًا من `tauri dev` / `tauri build`) |

يجب أن يكون `veryagent-mcp` بجوار ملفه التنفيذي الأصلي وقت التشغيل — برامج التثبيت وصورة Docker ومُجمِّع sidecar الخاص بـ Tauri جميعها تضعه بجوار `veryagent` / `veryagent-server`. يمكن لعمليات البناء من المصدر والتخطيطات المخصّصة تجاوز البحث باستخدام متغير البيئة `VERYAGENT_MCP_BIN=/مسار/مطلق/veryagent-mcp`. في حال غياب الرفيق، يتم تخطّي التفويض (مع تسجيل تحذير واحد) وتستمر باقي جلسة الوكيل في العمل.
...
# تطبيق سطح المكتب الكامل (Tauri + Next.js، يبني sidecar الخاص بـ veryagent-mcp تلقائيًا)
pnpm tauri dev

# بناء إصدار سطح المكتب (يُضمِّن veryagent-mcp بوصفه externalBin)
pnpm tauri build

# خادم مستقل (بدون Tauri/واجهة رسومية)
pnpm server:dev
pnpm server:build                  # ملف الإصدار التنفيذي ضمن src-tauri/target/release/veryagent-server

# بناء رفيق veryagent-mcp بشكل صريح (لثلاثية المضيف)
pnpm tauri:prepare-sidecars        # الناتج: src-tauri/binaries/veryagent-mcp-<triple>
...
cargo check --no-default-features --bin veryagent-server            # وضع الخادم
cargo check --no-default-features --bin veryagent-mcp               # رفيق MCP
cargo clippy --all-targets --features test-utils -- -D warnings

# اختبارات Rust
cargo test --features test-utils                                # سطح المكتب (يشمل التكامل)
cargo test --no-default-features --bin veryagent-server --lib       # وضع الخادم
cargo insta review                                              # قبول تحديثات لقطات المُحلِّل
```

> نصيحة: عند توفّر بناء جديد لـ `veryagent-mcp` ضمن `src-tauri/target/release/` وأردت توجيه `veryagent-server` مُشغَّل يدويًا إليه دون إعادة التثبيت، صدِّر `VERYAGENT_MCP_BIN=$(pwd)/src-tauri/target/release/veryagent-mcp`.

### نشر الخادم

يمكن تشغيل VeryAgent كخادم ويب مستقل بدون بيئة سطح مكتب.
...
الملفات التنفيذية المُعدّة مسبقًا (مع موارد الويب المضمّنة) متاحة في صفحة [Releases](https://github.com/plhys/veryagent-plus/releases):
...
cargo build --release --bin veryagent-mcp --no-default-features    # رفيق التفويض
VERYAGENT_STATIC_DIR=../out ./target/release/veryagent-server          # يتم التقاط veryagent-mcp بوصفه ملفًا شقيقًا
```

إذا احتفظت بالملفين التنفيذيين في دليلين منفصلين، فاضبط `VERYAGENT_MCP_BIN=/مسار/مطلق/إلى/veryagent-mcp` حتى يستطيع التشغيل العثور على الرفيق؛ بدون ذلك، يُعطَّل التفويض متعدد الوكلاء بصمت.
...
| ------------------------------ | ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `VERYAGENT_PORT`                   | `3080`                 | منفذ HTTP                                                                                                                                                                                                                                                                                                                                                                                                   |
| `VERYAGENT_HOST`                   | `0.0.0.0`              | عنوان الربط                                                                                                                                                                                                                                                                                                                                                                                                 |
| `VERYAGENT_TOKEN`                  | _(عشوائي)_             | رمز المصادقة (يُطبع في stderr عند البدء)                                                                                                                                                                                                                                                                                                                                                                    |
| `VERYAGENT_DATA_DIR`               | `~/.local/share/veryagent` | دليل قاعدة بيانات SQLite (والجذر أيضاً لـ `uploads/` و `pets/`)                                                                                                                                                                                                                                                                                                                                             |
| `VERYAGENT_STATIC_DIR`             | `./web` أو `./out`     | دليل التصدير الثابت لـ Next.js                                                                                                                                                                                                                                                                                                                                                                              |
| `VERYAGENT_MCP_BIN`                | _(غير مُحدّد)_         | المسار المطلق لرفيق `veryagent-mcp`. يتجاوز البحث الافتراضي (ملف شقيق للملف التنفيذي + `PATH`). استخدمه لعمليات البناء من المصدر أو التخطيطات المخصّصة التي يقع فيها الرفيق خارج دليل تثبيت الخادم.                                                                                                                                                                                                            |
| `VERYAGENT_SKIP_SIDECAR`           | _(غير مُحدّد)_         | متغير راحة للواجهة الأمامية فقط لـ `pnpm tauri dev` / `pnpm tauri build` — عند `1` يتم تخطّي بناء sidecar الخاص بـ `veryagent-mcp`. يُعطَّل التفويض في هذا البناء؛ ويجب ترك المتغير غير مُحدَّد للقطع الصالحة للشحن.                                                                                                                                                                                            |
| `VERYAGENT_UPLOAD_MAX_TOTAL_BYTES` | _(غير مُحدّد)_         | حدّ صارم لإجمالي البايتات المقيمة تحت `<data dir>/uploads/`. عدد بايتات عشري (مثلاً `10737418240` لـ 10 GiB). إذا كان غير مُحدّد أو `0` أو قيمة لا يمكن تحليلها فسيتم تعطيل الحدّ وطباعة سطر عند البدء حتى تكون الحالة مرئية. يُطبَّق الحدّ داخل عملية `veryagent-server` واحدة — تحتاج عمليات النشر الموسَّعة أفقياً التي تتشارك حجم `uploads/` واحداً إلى تنسيق خارجي (قفل ملف، Redis، حصّة عبر بروكسي عكسي). |
| `VERYAGENT_UPLOAD_QUOTA_STRICT`    | _(غير مُحدّد)_         | عند كونه صحيحاً (`1` / `true` / `yes` / `on`)، يُلغي البدء برمز خروج 2 إذا كانت `VERYAGENT_UPLOAD_MAX_TOTAL_BYTES` مضبوطة على قيمة لا يمكن تحليلها، بدلاً من المتابعة مع تحذير WARN. استخدم هذا حين تتطلب سياستك الأمنية أن «تكون الحصّة المُعدَّة فعّالة».                                                                                                                                                     |
...
- إذا كان VeryAgent مفيداً لك، يمكنك دعوتي إلى فنجان قهوة

<img src="../images/weixin-sponsor-light.jpg#gh-light-mode-only" alt="دعم VeryAgent" width="240" />
<img src="../images/weixin-sponsor-dark.jpg#gh-dark-mode-only" alt="دعم VeryAgent" width="240" />

## شكر وتقدير

- [ACP](https://agentclientprotocol.com) — بروتوكول Agent Client (ACP) هو الأساس الذي يمكّن VeryAgent من الاتصال بعدة وكلاء
- [Superpowers](https://github.com/obra/superpowers) — يُشغِّل وحدة مهارات الخبراء في VeryAgent
- [OfficeCLI](https://github.com/iOfficeAI/OfficeCLI) — يُشغِّل سير عمل مستندات Office في VeryAgent
